  ...
  def addAccessories(ibox_id)
    ...
    for i in 0..res.length/12-1
      ...
      if (!isSavedAccessory(ibox_id, res[0+12*i].to_s.split('=')[1])) 
        ...
        name = @accessory.name #borro los espacios para pasarselo al webservice
        respuesta = iboxExecute(@ibox.ip, @ibox.port, '/cgi-bin/Set.cgi?ZID=' + @accessory.zid + '&ALIAS=' + name.delete(' ') + '&X=' + @accessory.x  + '&Y=' + @accessory.y + '&W=' + @accessory.w + '&H=' + @accessory.h + '&Layer=0',@ibox.user,@ibox.password)
        logger.debug "############################ respuesta = #{respuesta}"
        logger.debug "############################ name = #{name.delete(' ')}"
